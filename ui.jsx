...
function PageHeader({ title, subtitle, actions }) {
  return (
    <div style={{
      padding: "28px 32px 20px", display: "flex", alignItems: "flex-start", gap: 24,
      borderBottom: "0.5px solid var(--divider)",
    }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 500, letterSpacing: "-0.02em", lineHeight: 1.2 }}>{title}</h1>
        {subtitle && <div style={{ marginTop: 4, color: "var(--dim)", fontSize: 13 }}>{subtitle}</div>}
      </div>
