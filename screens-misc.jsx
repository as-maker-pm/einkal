        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 6 }}>
          {srcs.map((c, i) => (
            <span key={c.id} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
              ...
              <span style={{ fontSize: 12, color: "var(--text-2)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 110 }}>{c.name}</span>
            </span>
          ))}
          <span style={{ color: "var(--dim)", display: "flex", alignItems: "center", padding: "0 2px" }}>
            {flow.direction === "two-way" ? <I.ArrowsLR size={12} /> : <I.ArrowRight size={12} />}
          </span>
          {dsts.map((c, i) => (
            <span key={c.id} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
              {i > 0 && <span style={{ color: "var(--muted)", fontSize: 11 }}>+</span>}
